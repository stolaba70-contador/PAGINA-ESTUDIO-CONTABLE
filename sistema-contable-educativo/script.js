...
function renderESP(container, saldosCuentas, modelLabel) {
  ...
  (ELEMENTOS['3'].subLabels || ['_']).forEach(sub => {
    (RUBROS['3'][sub] || []).forEach(rName => {
      const cuentasEnRubro = cuentas.filter(c => c.elemento === '3' && c.correncia === sub && c.rubro.toLowerCase() === rName.toLowerCase());
      cuentasEnRubro.forEach(c => {
        const saldoNeto = saldosCuentas[c.nombre.toLowerCase()] || 0;
        const valor = saldoNeto * -1;
        if (valor !== 0) {
          html += `<tr class="esp-rubro-row PN-rubro"><td>${c.nombre}</td><td style="text-align:right;">${formatMoney(valor)}</td></tr>`;
          totalPN += valor;
        }
      });
    });
      ...
      <table style="width:100%; border-collapse:collapse; min-width:1100px; font-size:12px;">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td style="${tdBoldR}">${fmt(ajusteCapital + get('Ajuste de capital p/ mantenimiento de la Capacidad Operativa'))}</td>
